...
## 2026-09-13
### Added
- You can now obtain 16 blackstone by crafting it from 1 obsidian (survival)
...
## 2026-09-05
### Added
- You can now craft Fishing Rods with Unbreaking through the `/menu` (survival)
...
## 2026-08-23
### Changed
- Slightly more monsters might spawn now. I "merely" changed the settings to match what PaperMC's default is, so this should hopefully not be too bad (survival)
...
## 2026-08-11
### Removed
- XP orbs can no longer drop from any source (survival)
### Added
- Bottle o' Enchanting can now be crafted in Extreme Mode (survival)
...
## 2026-07-28
### Removed
- Resized/Differently scaled mobs that spawn naturally were removed as the community decided against them. They felt pretty unique at first but the fact that they intentionally broke lots of mob farms as well as leading to mobs sometimes glitching out of where they shouldn't was not desired (survival)

## 2026-07-21
### Changed
- Server now runs 26.2 (creative)
### Removed
- Arceon was removed as it's no longer compatible with 26.2 and I don't exactly like the idea of paying for it again and again just to keep support :( (creative)
...
- 3 more endeavors for new players to get started a little more easily and to boost their morale (survival)
### Removed
...
- All vanilla enchantments which you could get from villagers are now available in the Vanilla Enchanter section of the Server Shop... except for Mending of course :P (survival)
### Changed
- Ender Chests can be opened in the entire overworld again. For other worlds, the rules remain unchanged (survival)
- Sleeping percentage for the overworld was changed to 75% so you can sleep once again. Since the levelled mobs world is always night, perhaps we can loosen up the old, hard rules around the overworld :) (survival)
...
- Armor Stand Editor can now be used by everyone from the start, removing the need of a quest (survival)
...
## 2026-06-25
### Added
- You can now buy Vanilla Enchanted books in the server shop. We're starting with Silk Touch and Sharpness 5 for a start and will see how high demand is and if the price of 5 MGC seems right (survival)
...
### Added
- You now earn MetaCoins for every skill XP you earn. The formula is quite simple: `0.0003*xp*power+0.1` xp stands for how many xp you earned, power is all your skill levels combined. So in simpler terms: Try to level up all skills so you earn more money. This should also make newbies realize early on that levelling up skills makes money (survival)
...
- Endeavors are now proper quests! Turns out what I thought wasn't possible with the quests plugin **is** possible and I just failed to realize (survival)
...
## 2026-05-20
### Added
- `/hardcore` was added. This is a permanent switch to turn on hardcore where you're banned from the server for 7 days if you die (survival)

## 2026-05-18
### Removed
- Hardmode was removed as it will hopefully be replaced by the Extreme world(s) soon (survival)
...
### Added
- Allowance 4 was added to the rules, stating that it's okay to steal from graves in survival without having to expect any consequences from the staff team (survival)
...
- The auto restart plugin was replaced with one that is compatible with 26.1 (lobby, survival, creative)
...
## 2026-04-26
### Added
- Custom Tools were added to the Server Shop. This includes hoes, axes, pickaxes and shovels. Some of those models weren't available before. (survival)
...
## 2026-04-22
### Added
- The last few purchases for custom tools were added which concludes the migration from crates to purchases \\o/ (survival)
...
## 2026-04-06
### Fixed
- Custom enchantments did not work on copper armor, that's now been fixed (survival)

## 2026-04-04
### Changed
- Hardmode S now gives the hunger II effect, Harmode M gives the hunger III effect instead of hunger I as mentioned before. Meaning your Hardmode decides on how high the hunger level is that you get permanently (survival)
...
## 2026-04-01
### Added
- You now get the hunger effect when breaking or placing blocks. We shall see how that affects the food demand and prices etc. (survival)
...
## 2026-03-29
### Fixed
- Exporting a GoBrush now works. The item got the force enchantment glint component added which was not whitelisted in the illegal item NBT (creative)
...
- The prevention of mounting leashed mobs was also changed due to the same reason as above (survival)

## 2026-03-22
### Added
- Fishing now has a chance to give the Wither effect if you have Hardmode enabled (survival)
...
## 2026-03-16
### Added
- Vegan Pufferfish and Vegan Rabbit Meat can now be bought in the server shop to allow our vegan friends to have an alternative to killing animals (survival)

## 2026-03-15
### Changed
- You are now kicked 10 minutes after being counted as AFK instead of 45 minutes as it was previously. This was mainly done to prevent excessive AFK farms (survival)
...
- Creating a new town now costs 10k MetaCoins (survival)
### Added
...
## 2026-03-11
### Changed
- Iron Golems no longer drop items on death. This was mainly done to make resources a little harder to obtain, more details can be found on Discord's announcement channel (survival)
...
## 2026-03-07
### Changed
- Elites now have more health. This was done to put a damper on inflation (survival)
...
## 2026-03-01
### Removed
- Leaderboard for Challenges was removed as it was causing lag and also I want to change challenges in the near future to allow for even more challenges but without a focus on competition (survival)
...
## 2026-02-22
### Added
- Bedrock Edition users can now use '/geyser advancements' to see their advancements (survival)
- Bedrock Edition users can now use '/geyser offhand' to put any item in their offhand (survival)
...
## 2026-02-20
### Changed
- Personal Difficulties AKA Pick Mode now only lock you for 3 real life days rather than 7. This was mainly done as we realized Fighting and Archery do not benefit from the higher difficulties and this way you can switch more easily (survival)
- There is now a hint that the Fighting and Archery skills do not benefit from Hard or Extreme Mode (survival)
...
## 2026-01-31
### Added
- Server Shop now sells passive mobs of varying sizes. Currently available are sheep and rabbits with more to potentially follow depending on the requests by players (survival)
...
## 2026-01-24
### Added
- Using either `/sethome` or `/set home` will now give you the info to check out `/home help` (creative)

## 2026-01-21
### Changed
- Wandering Traders now give clearer, more accessible messages when interacted with (survival)
...
## 2026-01-12
### Changed
- Everything inside the Server Shop now sells for MetaCoins instead of MetaTokens. Prices are simply the amount of MetaTokens * 2000 in MetaCoins (survival)
...
## 2026-01-03
### Added
- You can now use `/trade <user>` to trade with others. This only works if both players are standing in a town chunk however. You can trade items as well as MetaCoins (survival)
...
- The crate named "Diamond Pickaxe Crate 1" now costs 3 Diamonds and 4 MetaTokens (survival)
- Unbreaking IV now costs 5 MetaTokens as the first enchantment to try out the MetaToken system with (survival)
### Removed
...
- You now have a chance to earn a MetaTokens from generating loot and if you do, people near you will also get one (survival)
